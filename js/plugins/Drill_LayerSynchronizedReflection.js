 ...
 *   事件、玩家、跟随队员都有效。
 * 2.注意区分 镜面 和 镜像 关系。
 * 3.不反射镜像 或者 没有在镜面上 只是镜像不显示，并不是不存在。
 *   如果你要完全关闭镜像，需要添加地图备注来禁用整张地图的镜像。
 * 4.同步镜像能支持大部分动作效果，包括跳跃、透明度、多帧行走图。
 * 5.所有事件都默认开启镜像反射，并且与事件的透明度一致，通过事件复制器
 *   生成的新事件也支持镜像反射。
 * 6.反射遇到循环地图的边缘时，会出现镜像关闭的现象。
 * 7.反射不支持滤镜、粉碎效果。
 * 8.该插件可以与 倒影镜像 插件同时使用，但是同步镜像是没有倒影的。
 *
 * 9.同步镜像是根据反射的基准线，同步反射出来镜像。
 *   相当于墙壁上的大镜子的垂直镜像。而不是倒影。
 * 10.你朝向上（面向镜面）时，镜像是朝向下的。你可以设置注释锁定镜像的朝向。

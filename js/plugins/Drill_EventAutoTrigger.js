...
/*:
 * @plugindesc [v1.6]        物体触发 - 固定区域 & 玩家接近 & 条件触发
 * @author Drill_up
 *
 * @param 是否修正区域判定
 * @type boolean
 ...
 * 3.被触发是一个点，主动触发是一个范围，范围内符合条件的点会被触发。
 * 4.玩家触发比较特殊，触发是连续不间断的。
 * 5.你可以自定义条件关键字，用于连接玩家与不同的被触发的事件。
 * 6.触发独立开关后都是设置为ON。硬性规定。
 * （程序上可以设置范围OFF独立开关，但是OFF情况更复杂，会使得事件页变混乱。）
 * 7.该插件主要用于自动触发接近玩家区域的事件。
 *
 * -----------------------------------------------------------------------------
 * ----激活条件 - 被触发

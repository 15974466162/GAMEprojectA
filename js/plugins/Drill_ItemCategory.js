...
　　var DrillUp = DrillUp || {}; 
	...
	if(DrillUp.parameters['物品界面物品类型'] != "" ){
		...
		DrillUp.g_ICa_type_iSence = ["物品","武器","防具","重要物品"];
	}
	if(DrillUp.parameters['商店出售物品类型'] != "" ){
		DrillUp.g_ICa_type_sSence = JSON.parse(DrillUp.parameters['商店出售物品类型']);
	}else{
		DrillUp.g_ICa_type_sSence = ["物品","武器","防具","重要物品"];
	}
	
	DrillUp.g_ICa_type_max = 10;
	DrillUp.g_ICa_type = [];
	
	for (var i = 0; i < DrillUp.g_ICa_type_max; i++) {
